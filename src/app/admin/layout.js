...
import {
  ...
  ChevronRight, Briefcase, X, ExternalLink, Clock, BarChart3
} from 'lucide-react';
...
export default function AdminLayout({ children }) {
  ...
  useEffect(() => {
    const unsub = on('new_application', (payload) => {
      ...
      toast.custom((t) => (
        <div className={`${t.visible ? 'animate-slide-down' : ''} bg-white rounded-xl shadow-lg border border-brand-100 p-4 flex items-start gap-3 max-w-sm cursor-pointer`}
          onClick={() => {
            toast.dismiss(t.id);
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              <span className="font-medium text-slate-700">{payload.applicant}</span> applied for {payload.job}
            </p>
  ...
  const Sidebar = () => (
    <div className="w-56 h-full flex flex-col bg-slate-900">
                ...
                <div className="absolute right-0 top-11 w-80 bg-white rounded-2xl shadow-xl border border-slate-200 z-50 overflow-hidden animate-slide-down">
                  ...
                  <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 bg-slate-50">
                    ...
                    <div className="flex items-center gap-2">
                      {notifications.length > 0 && (
                        <button onClick={clearAll} className="text-[10px] text-slate-400 hover:text-slate-600 font-medium">Clear all</button>
                      )}
                      <button onClick={() => setBellOpen(false)} className="p-1 rounded-lg hover:bg-slate-200 transition-colors">
                  ...
                  <div className="max-h-96 overflow-y-auto">
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(notif => (
                        <div
                          key={notif.id}
                          className={`px-4 py-3 border-b border-slate-50 hover:bg-slate-50 transition-colors cursor-pointer ${!notif.read ? 'bg-brand-50/40' : ''}`}
                          onClick={() => {
                            markRead(notif.id);
                            setBellOpen(false);
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${notif.isIntern ? 'bg-amber-100' : 'bg-brand-100'}`}>
                              {notif.isIntern
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5">
                                <p className="text-sm font-semibold text-slate-800 truncate">{notif.applicant}</p>
                                {!notif.read && <span className="w-1.5 h-1.5 bg-brand-500 rounded-full shrink-0" />}
                              </div>
                              <p className="text-xs text-slate-500 truncate">Applied for {notif.job}</p>
                              <div className="flex items-center gap-1 mt-1">
                                <Clock size={10} className="text-slate-300" />
                                <p className="text-[10px] text-slate-400">{format(new Date(notif.time), 'MMM d, h:mm a')}</p>
                              </div>
                            </div>
                  ...
                  {notifications.length > 0 && (
                    <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50">
                      <Link href="/admin/applications" onClick={() => setBellOpen(false)}
